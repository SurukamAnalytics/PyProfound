...
class Data:
    ...
    def extract_ids(self):
        ...
        if "id" in first_column or "no" in first_column or "number" in first_column:
            while True:
                id_input = raw_input("Is %s the observation id/number?(y/n): " % (str(self.train.columns[0])).lower())
        ...
        if id_flag == 0:
            while True:
                id_column = raw_input("Please enter column name of id (or type none if no such column exists): ")
                print id_column
                if id_column.lower().strip() != "none" and id_column not in self.train.columns.values:
    ...
    def extract_target(self):
        ...
        for colname in self.train.columns.values:
            if colname.lower() in ["response","result","target"]:
                while True:
                    target_input = raw_input("Is %s the target variable?(y/n): " % (colname))
        ...
        if target_flag == 0:
            while True:
                target_column = raw_input("Please enter column name of target variable (or q to quit): ")
                if target_column == "q":
                    sys.exit()
...
file_path1 = raw_input("Please enter the file path of test file: ")
df = pd.read_csv(file_path)
df1= pd.read_csv(file_path1)
cleaned_data = Data(df,df1)
x,y = cleaned_data.clean_data()
#print(x)
#print(y)
x.to_csv('out.csv', sep=',', encoding='utf-8')
y.to_csv('out1.csv', sep=',', encoding='utf-8')